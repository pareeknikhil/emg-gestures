...
spec_emg_path = os.environ.get('SPEC_EMG_FILE')
...
def parse_tfrecord_fn(example_proto):
    ...
    window.set_shape([int(TIMESTEPS/2)+1, num_emg_channels])
...
def load_dataset(selected_type):
    return load_tfrecord(filename=f"{tfrecord_path}/{selected_type}/{spec_emg_path}")
...
def run_spec() -> None:
    ...
    mlflow.set_experiment(experiment_name='emg-spec-embedding')
    with mlflow.start_run() as run:
        ...
        inputs = Input(shape=(int((TIMESTEPS/2)+1), num_emg_channels))
        x = layers.Conv1D(32, 5, strides=2, activation='relu', padding='same')(inputs)
        x = layers.MaxPooling1D(2)(x)
        x = layers.Conv1D(32, 5, strides=2, activation='relu', padding='same')(x)
        x = layers.GlobalAveragePooling1D()(x)
        x = layers.Dense(LATENT_DIM, activation = 'relu', name='conv_final')(x)
        outputs = layers.Dense(units=get_num_labels(), activation='softmax')(x)

        model = Model(inputs, outputs)
        model.compile(optimizer=optimizers.Adam(learning_rate=LEARNING_RATE), loss='categorical_crossentropy', metrics=['accuracy'])

        encoder_model = Model(inputs=model.input, outputs=model.get_layer('conv_final').output)
